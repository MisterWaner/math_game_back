...
cron.schedule("0 0 * * *", () => {
    ...
    users.forEach((user) => {
        db.prepare(
            "UPDATE users SET weekly_score = weekly_score + daily_score, daily_score = 0 WHERE id = ?"
        ).run(user.id);
    });
...
cron.schedule("0 0 * * 0", () => {
    ...
    users.forEach((user) => {
        db.prepare(
            "UPDATE users SET monthly_score = monthly_score + weekly_score, weekly_score = 0 WHERE id = ?"
        ).run(user.id);
    });
...
cron.schedule("0 0 1 * *", () => {
    const users = fetchUsers();
    users.forEach((user) => {
        db.prepare("UPDATE users SET monthly_score = 0 WHERE id = ?").run(
            user.id
        );
    });
